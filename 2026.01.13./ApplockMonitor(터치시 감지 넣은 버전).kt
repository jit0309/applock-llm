...
//    private var isInHomeProtection = false
    ...
    private fun sanitizePackage(pkg: String?): String? {
        ...
        return if (SAFE_PACKAGES.contains(pkg)) pkg else null
    }

    private var lastTouchCheckTime = 0L
    private val TOUCH_CHECK_INTERVAL = 5000L // 5초
...
//        // 개선된 코드 (30초마다 체크, 50개씩 배치)
//        usageEventsMonitor?.startMonitoring(
//            intervalMs = 30000L,  // 30초마다 체크
//            batchSize = 50        // 50개 모이면 저장
//        )
        Log.d(TAG, "✅ UsageEvents monitoring started")
    }
    ...
    private fun startUsageEventsMonitoring() {
        ...
        // 방법 1: 모든 이벤트 모니터링 (5초마다)
        usageEventsMonitor?.startMonitoring(intervalMs = 2 * 1000L)
...
//                    }
    ...
    override fun onAccessibilityEvent(event: AccessibilityEvent?) {
        ...
        // 🔽 COUNT_UP(적립) 또는 IDLE(버그)일 때만 터치 백업
        if (event?.eventType == AccessibilityEvent.TYPE_VIEW_CLICKED) {
            // COUNT_DOWN(정상 사용중)이면 체크 불필요
            if (!isAccumulatingPoints && timer.getCurrentMode() != TimerMode.IDLE) {
                return
            }

            val currentTime = System.currentTimeMillis()
            if (currentTime - lastTouchCheckTime >= TOUCH_CHECK_INTERVAL) {
                lastTouchCheckTime = currentTime
                checkCurrentAppViaUsageStats()
            }
            return
        }
...
//테스트용
        ...
        // 👇 키보드 이벤트는 아예 무시!
        if (packageName == "com.samsung.android.honeyboard") {
            Log.d("StateLog", "키보드 이벤트 무시")
            return
        }
...
//                            // 여기에 보호 시간 코드 추가
//                            isInHomeProtection = true
//                            Handler(Looper.getMainLooper()).postDelayed({
//                                isInHomeProtection = false
//                                Log.d("StateLog", "홈 화면 보호 시간 종료")
//                            }, 300) // 300ms로 짧게 설정
                        }
                ...
                packageName != null && !EXCLUDED_PACKAGES.contains(packageName) -> {
                    ...
                    if (isHomeTransition && (currentTime - lastHomeTransitionTime) < 500) {
                        Log.d("StateLog", "홈 전환 후 YouTube 이벤트 무시")
                        return
                    }
...
//            }
    ...
    private fun handleAppChange(newApp: String) {
        if (isMonitoring) {
            ...
            if (!isAccumulatingPoints && !EXCLUDED_PACKAGES.contains(newApp)) {
...
//            }
    ...
    private fun showLockScreen() {
        ...
        Log.d("LockScreenTrace", "========================================")
        Log.d("LockScreenTrace", "showLockScreen 호출됨!")
        Log.d("LockScreenTrace", "호출 위치: ${caller?.className}.${caller?.methodName}:${caller?.lineNumber}")
        Log.d("LockScreenTrace", "현재 상태:")
        Log.d("LockScreenTrace", "  - isLockScreenShowing: $isLockScreenShowing")
        Log.d("LockScreenTrace", "  - currentActiveApp: $currentActiveApp")
        Log.d("LockScreenTrace", "  - isAccumulatingPoints: $isAccumulatingPoints")
        Log.d("LockScreenTrace", "  - availableTime: ${timer.timerState.value.availabletime}")
        Log.d("LockScreenTrace", "  - timerMode: ${timer.getCurrentMode()}")
        Log.d("LockScreenTrace", "========================================")
...
//                                notificationManager.notify(4, notificationBuilder.build())
    ...
    // UsageStats를 통해 현재 앱 확인 (TYPE_VIEW_CLICKED 백업용)
    private fun checkCurrentAppViaUsageStats() {
        try {
            ...
            Log.d(TAG, "Touch backup check - Current app: $packageName")
            ...
            if (packageName != null &&
                ...
                when {
                    // 적립모드: 잠금
                    isAccumulatingPoints -> {
                        Log.d("TouchBackup", "🔒 $packageName")
                        currentActiveApp = packageName
                        showLockScreen()
                    }
                    // IDLE 버그: 타이머 복원
                    timer.getCurrentMode() == TimerMode.IDLE -> {
                        Log.d("TouchBackup", "▶️ $packageName")
                        currentActiveApp = packageName
                        timer.setMode(TimerMode.COUNT_DOWN)
                    }
                }
            ...
            Log.e(TAG, "Error checking current app via UsageStats: ${e.message}", e)
        }
    }
}
