...
//    }
    ...
    // ✅ 배치로 여러 이벤트를 한 번에 로깅
    fun logUsageEventsBatch(userId: String, events: List<UsageEventLogData>) {
        val updates = mutableMapOf<String, Any>()

        events.forEach { eventData ->
            val eventMap = mapOf(
                "Idx" to eventData.idx,
                "AppName" to eventData.appName,
                "AppPackage" to eventData.appPackage,
                "EventType" to eventData.eventType,
                "EventTime" to formatTimestamp(eventData.eventTime)
            )
            updates["usage_events/$userId/${eventData.idx}"] = eventMap
        }

        database.updateChildren(updates)
            .addOnSuccessListener {
                Log.d("FirebaseManager", "✅ Batch logged ${events.size} usage events")
            }
            .addOnFailureListener { error ->
                Log.e("FirebaseManager", "❌ Failed to batch log usage events: ${error.message}")
            }
    }

    //accessability lockdebug
    fun logAccessibilityEvent(userId: String, eventData: AccessibilityEventData) {
        val eventMap = mapOf(
            "Timestamp" to formatTimestamp(eventData.timestamp),
            "EventType" to eventData.eventType,
            "PackageName" to (eventData.packageName ?: "null"),
            "ClassName" to (eventData.className ?: "null"),
            "WindowId" to (eventData.windowId ?: -1),
            "Action" to (eventData.action ?: -1),
            "IsFullScreen" to eventData.isFullScreen
        )

        database.child("accessibility_debug").child(userId)
            .child(formatTimestamp(eventData.timestamp))
            .setValue(eventMap)
            .addOnFailureListener { e ->
                Log.e("FirebaseManager", "Failed to log accessibility event: ${e.message}")
            }
    }
}
...
// 모든 UsageEvent 로깅을 위한 데이터 클래스
data class UsageEventLogData(
    val idx: Long = 0L,                    // 이벤트 인덱스
    val appName: String = "",              // 앱 이름 (필요시)
    val appPackage: String = "",           // 앱 패키지명
    val eventType: String = "",            // 이벤트 타입 (ACTIVITY_PAUSED, ACTIVITY_RESUMED 등)
    val eventTime: Long = 0L               // 이벤트 발생 시간
)
