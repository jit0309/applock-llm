...
//            "_timestamp" to ServerValue.TIMESTAMP  // 서버 시간 기록
    ...
    // 2. 세션 데이터
    fun saveSessionData(userId: String, sessionData: SessionData) {
        val formattedStartTime = formatTimestamp(sessionData.startTime)

        val sessionMap = mutableMapOf(
            "StartTime" to formattedStartTime,
            "accumulatedTime" to sessionData.accumulatedTime,
            "bonusApplied" to sessionData.bonusApplied
        )
        // endTime이 0이 아닐 때만 포함
        if (sessionData.endTime != 0L) {
            sessionMap["EndTime"] = formatTimestamp(sessionData.endTime)
        }

        database.child("sessions").child(userId)
            .child(formattedStartTime)  // 시작 시간을 키로 사용
            .setValue(sessionMap)
            .addOnSuccessListener {
                database.child("sessions").child(userId).keepSynced(true)
            }
    }

    // 3. 포인트 사용 기록
    fun logPointUsage(userId: String, pointData: PointUsageData) {
        val actualUsedPoints = pointData.pointsUsed - pointData.returnedPoints

        val pointMap = mapOf(
            "StartTime" to formatTimestamp(pointData.startTime),
            "EndTime" to formatTimestamp(pointData.endTime),
...
//            "_timestamp" to ServerValue.TIMESTAMP,  // 서버 시간 기록
        ...
        database.child("point_usage").child(userId)
            .child(formatTimestamp(pointData.startTime))
            .setValue(pointMap)
            .addOnSuccessListener {
                // 로컬 캐시 업데이트
...
//            "_timestamp" to ServerValue.TIMESTAMP,  // 서버 시간 기록
    ...
    // 7. 포인트모드에서 5분 미사용시 적립모드로 넘어간 로그
    fun logInactivityTransition(userId: String, data: InactivityTransitionData) {
        val transitionMap = mapOf(
            "Timestamp" to formatTimestamp(data.timestamp),
            "RemainingPoints" to formatSeconds(data.remainingPoints),
//            "_timestamp" to ServerValue.TIMESTAMP,  // 서버 시간 기록
        )
        database.child("inactivity_transitions").child(userId)
            .child(formatTimestamp(data.timestamp))
            .setValue(transitionMap)
            .addOnSuccessListener {
                // 로컬 캐시 업데이트
                database.child("inactivity_transitions").child(userId).keepSynced(true)
            }
    }
