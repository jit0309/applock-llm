            ...
            if (isButtonClickable) {
                ...
                if (state.availabletime > 0) {
                    ...
                    if (sessionStartTime > 0) {
                        ...
                        // Firebase에 세션 종료 정보 저장
                        val userId = android.provider.Settings.Secure.getString(contentResolver, android.provider.Settings.Secure.ANDROID_ID)
                        FirebaseManager.getInstance().saveSessionData(
                            userId, SessionData(
                                startTime = sessionStartTime,
                                endTime = currentTime,
                                accumulatedTime = ((currentTime - sessionStartTime) / 1000f).toInt().toFloat(),
