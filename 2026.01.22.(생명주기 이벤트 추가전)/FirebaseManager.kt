...
//    }
    ...
    // ✅ 배치로 여러 이벤트를 한 번에 로깅
    // FirebaseManager.kt (재시도 로직 제거 버전)
    ...
    fun logUsageEventsBatch(userId: String, events: List<UsageEventLogData>) {
        ...
        events.forEach { eventData ->
            ...
            updates["usage_events/$userId/${formatTimestamp(eventData.eventTime)}"] = eventMap
        }

        database.updateChildren(updates)
            .addOnSuccessListener {
                Log.d("FirebaseManager", "✅ Batch logged ${events.size} events")
            }
            .addOnFailureListener { error ->
                Log.e("FirebaseManager", "❌ Batch log failed: ${error.message}")
            }
    }
