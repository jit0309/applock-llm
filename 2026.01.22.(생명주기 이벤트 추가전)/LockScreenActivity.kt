...
class LockScreenActivity : AppCompatActivity() {
            ...
            if (isButtonClickable) {
                ...
                if (state.availabletime > 0) {
                    ...
                    if (sessionStartTime > 0) {
                        val sessionHours = (state.sessionTime / 1800f).toInt()  // 30분 단위
                        val bonusTime = sessionHours * 300f  // 5분(300초) 보너스
    ...
    private fun updateBonusInfo() {
        ...
        // 여기가 중요한 변경 부분: 1800초(30분)로 나머지 연산을 수행하여 항상 0~1800 사이 값이 되도록 함
        ...
        bonusTimeText.text = String.format("%d분 %d초 후 5분 추가!", remainingMinutes, remainingSecondsInMinute)

        // 진행 상태에 따라 체크 마크 표시
        if (elapsedSeconds >= targetSeconds - 10) { // 거의 완료 시 체크 마크 표시 (30분에 몇 초 부족할 때)
