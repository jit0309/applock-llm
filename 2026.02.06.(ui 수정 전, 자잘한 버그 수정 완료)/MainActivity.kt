...
class MainActivity : AppCompatActivity() {
                ...
                if (intent?.action == AppLockMonitorService.ACTION_TIME_UPDATE) {
                    ...
                    when (currentMode) {
                        TimerMode.COUNT_UP -> {
                            ...
                            lockIcon.setImageResource(R.drawable.lock_im)
                        }
                        ...
                        TimerMode.COUNT_DOWN, TimerMode.IDLE -> {
                            ...
                            lockIcon.setImageResource(R.drawable.unlock_im)
                        }
                    }
    ...
    //보너스 시간 메인화면 바
    private val bonusUpdateHandler = Handler(Looper.getMainLooper())
    private lateinit var bonusUpdateRunnable: Runnable
...
//            btnViewUsageStats = findViewById(R.id.btn_view_usage_stats)
            ...
            when (currentMode) {
                TimerMode.COUNT_UP -> lockIcon.setImageResource(R.drawable.lock_im)
                TimerMode.COUNT_DOWN, TimerMode.IDLE -> lockIcon.setImageResource(R.drawable.unlock_im)
            }
            Log.d("MainActivity", "Initial icon set for mode: $currentMode")
...
//                            )
//                            FirebaseManager.getInstance().logPointUsage(userId, pointData, System.currentTimeMillis()) // 5분 미사용 전환 파이어베이스
                            // 변수 초기화
    ...
    private fun startBonusTimeUpdates() {
        bonusUpdateRunnable = object : Runnable {
            override fun run() {
                updateBonusInfo()
                bonusUpdateHandler.postDelayed(this, 1000) // 1초마다 업데이트
            }
        }
        bonusUpdateHandler.post(bonusUpdateRunnable)
    }

    /**
     * 보너스 정보 업데이트 (LockScreenActivity와 동일)
     */
    private fun updateBonusInfo() {
        val progressBar = findViewById<android.widget.ProgressBar>(R.id.bonusProgressBar)
        ...
        // ⭐ 포인트 사용 중이면 00:00:00으로 고정
        val currentMode = timer.getCurrentMode()
        if (currentMode == TimerMode.COUNT_DOWN || currentMode == TimerMode.IDLE) {
            currentProgressText.text = "00:00:00"
            progressBar.progress = 0
            bonusTimeText.text = "30분 후 5분 추가!"
            checkMark.visibility = android.view.View.GONE
            totalProgressText.text = "30분"
            return
        }
        ...
        // 1800초(30분) 단위로 순환
        val elapsedSeconds = (elapsedTimeMillis / 1000).toInt() % 1800
        ...
        // 30분(1800초) 기준으로 진행 상태 계산
        val targetSeconds = 1800
        ...
        if (remainingSeconds > 0) {
            bonusTimeText.text = "${remainingMinutes}분 ${remainingSecondsInMinute}초 후 5분 추가!"
            checkMark.visibility = android.view.View.GONE
        } else {
            bonusTimeText.text = "보너스 획득!"
            checkMark.visibility = android.view.View.VISIBLE
    ...
    override fun onDestroy() {
        ...
        try {
            ...
            bonusUpdateHandler.removeCallbacks(bonusUpdateRunnable)
        } catch (e: Exception) {
