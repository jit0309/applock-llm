...
class AppLockMonitorService : AccessibilityService() {
    companion object {
        ...
        private const val INACTIVITY_TIMEOUT = 5 * 60 * 1000L // 포인트사용모드에서 5분 미사용 시 적립모드로 전환 - 0.5는 30초, 5븐 5분
        ...
        if (event?.eventType == AccessibilityEvent.TYPE_WINDOW_STATE_CHANGED) {
            when {
                ...
                packageName != null && !EXCLUDED_PACKAGES.contains(packageName) -> {
                    // 홈 화면 전환 후 일정 시간 내에 발생한 YouTube 이벤트 무시 (예: 500ms)
                    if (packageName == "com.google.android.youtube" && isHomeTransition && (currentTime - lastHomeTransitionTime) < 500) {
    ...
    fun onTimerDepleted() {
        ...
        if (isPointUseSession && pointUsageSessionStartTime > 0) {
            ...
            val pointData = PointUsageData(
                startTime = pointUsageSessionStartTime,
                endTime = currentTime,
                        ...
                        if (timeSinceLastActivity >= INACTIVITY_TIMEOUT) {
                            ...
                            withContext(Dispatchers.Main) {
                                ...
                                if (isPointUseSession && pointUsageSessionStartTime > 0) {
                                    ...
                                    // 비활성화로 인한 전환 기록
                                    FirebaseManager.getInstance().logInactivityTransition(
                                        userId,
                                        InactivityTransitionData(
                                            timestamp = currentTime,
                                            remainingPoints = remainingTime,
                                            sessionDuration = currentTime - pointUsageSessionStartTime,
                                            originalPoints = initialSessionPointAmount
                                        )
                                    )
