...
class AppLockMonitorService : AccessibilityService() {
    companion object {
        ...
        private const val INACTIVITY_TIMEOUT = 0.1 * 60 * 1000L // 5 * 60 * 1000L 포인트사용모드에서 5분 미사용 시 적립모드로 전환 - 0.5는 30초, 5븐 5분
    ...
    override fun onCreate() {
        ...
        logCountdownEvent("SESSION_START", "boot")  // start 로그
    ...
    fun startMonitoring(time: Float, isAccumulating: Boolean = false) {
        ...
        if (isAccumulating) {
            timer.setMode(TimerMode.COUNT_UP)
            logCountdownEvent("POINT_END / SESSION_START")  // start 로그
                    ...
                    TimerMode.IDLE -> {
                        ...
                        if (timeSinceLastActivity >= INACTIVITY_TIMEOUT) {
                            ...
                            withContext(Dispatchers.Main) {
                                ...
                                isMonitoring = true
                                timer.setMode(TimerMode.COUNT_UP)
                                logCountdownEvent("POINT_END / SESSION_START", "5min inactive")  // end 로그
...
//                                notificationManager.notify(4, notificationBuilder.build())
    ...
    fun startCountdownSession() {
        logCountdownEvent("SESSION_START")
    }
