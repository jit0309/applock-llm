...
class AppLockMonitorService : AccessibilityService() {
    companion object {
        ...
        //usagestatsmanager을 통해 activity_resume 감지
        private lateinit var usageStatsManager: UsageStatsManager
        private var lastCheckTime = 0L
        private val checkJob: Job? = null
    }
...
//    private var isInHomeProtection = false
    ...
    override fun onCreate() {
        ...
        // ✅ lastCheckTime을 현재 시간으로 초기화 (과거 이벤트 무시)
...
//                                notificationManager.notify(4, notificationBuilder.build())
    ...
    private fun checkActivityResumed() {
        ...
        while (events.hasNextEvent()) {
            events.getNextEvent(event)  // 같은 객체에 덮어쓰기

            // ACTIVITY_RESUMED만 감지
            if (event.eventType == UsageEvents.Event.ACTIVITY_RESUMED) {
                val packageName = event.packageName

                Log.d(TAG, "ACTIVITY_RESUMED 감지: $packageName")
        ...
        //락 처리
        // 폴링 루프 끝난 뒤 중복 있으면 하나만 처리
        lastResumedPackage?.let { pkg ->
            handleActivityResumed(pkg)
        }
        lastCheckTime = currentTime
    }

    private fun handleActivityResumed(packageName: String) {
        // ✅ 추가: 이중 체크
        if (EXCLUDED_PACKAGES.contains(packageName) ||
            packageName == "com.example.applock") {
            Log.d(TAG, "제외 패키지 - 무시: $packageName")
            return
        }

        // ✅ 잠금화면이 이미 떠있으면 무시
        if (isLockScreenShowing) {
            Log.d(TAG, "이미 잠금화면이 떠있음 - 무시")
            return
        }

        // ✅ Firebase 로깅 (포인트 사용 모드)
        if (!isAccumulatingPoints) {
            val eventData = UsageEventData(
                timestamp = System.currentTimeMillis(),
                eventType = "POINT_USE_LAUNCH"
        ...
        // 적립 모드일 때 락
        if (isAccumulatingPoints && !isLockScreenShowing) {
            showLockScreen()
        }
        ...
        broadcastTimeUpdate(packageName)
        updateNotification()
    }
