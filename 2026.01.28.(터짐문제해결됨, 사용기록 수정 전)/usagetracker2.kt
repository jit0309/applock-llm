...
class UsageTracker(
    ...
    fun onAppChanged(newPackageName: String?) {
        ...
        if (newPackageName != null) {
            Log.d(TAG, "🔄 App changed to: $newPackageName")
        }
    }
...
//                "endTime" to endTime,
            ...
            // Add app usage data
            val appsData = mutableMapOf<String, Long>()
            usageMap.forEach { (pkg, duration) ->
                if (duration > 0) {  // Only save apps with actual usage
                    appsData[sanitizeKey(pkg)] = duration
                }
            }
