...
//                    AccessibilityEvent.TYPE_VIEW_CLICKED  // 터치 이벤트 백업용
    ...
    override fun onAccessibilityEvent(event: AccessibilityEvent?) {
        ...
        //최우선: CONTENT 이벤트는 즉시 처리하고 return!
        if (event?.eventType == AccessibilityEvent.TYPE_WINDOW_CONTENT_CHANGED) {
            if (timer.getCurrentMode() == TimerMode.COUNT_DOWN) {
                ...
                scheduleContentCheck(packageName, className, currentTime)
            }
            return  // ✅ 다른 로직 전혀 실행 안 함!
        }
...
//            handleAppChange(packageName)
    ...
    private fun scheduleContentCheck(packageName: String, className: String, eventTime: Long) {
        ...
        contentCheckRunnable = Runnable {
            try {
                // ✅ COUNT_DOWN 모드 재확인 (추가)
                if (timer.getCurrentMode() != TimerMode.COUNT_DOWN) {
                    Log.d(TAG, "⏱️ CONTENT 체크 취소: COUNT_DOWN 모드 아님")
                    return@Runnable
                }

                Log.d(TAG, "✅ CONTENT 체크 실행: $packageName")  // ← 추가
