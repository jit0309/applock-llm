...
class AppLockMonitorService : AccessibilityService() {
    companion object {
        ...
        @JvmStatic
        @Synchronized
        fun checkAndUpdateLockTime(context: Context): Boolean {
            ...
            if (now - globalLastLockTime < LOCK_DEBOUNCE_MS) {
                Log.d(TAG, "⏱️ 디바운스 차단 (메모리): ${now - globalLastLockTime}ms 간격")
                return false
            }
            ...
            if (now - savedTime < LOCK_DEBOUNCE_MS) {
                Log.d(TAG, "⏱️ 디바운스 차단 (디스크): ${now - savedTime}ms 간격")
            ...
            prefs.edit().putLong(PREFS_LAST_LOCK_TIME, now).commit()  // ✅ apply 대신 commit
            Log.d(TAG, "✅ 디바운스 통과: 잠금화면 허용")
            return true
        }
    }
...
//    private var usageEventsMonitor: UsageEventsMonitor? = null
    ...
    fun onLockScreenClosed() {
        ...
        // ✅ 핵심 수정: currentActiveApp을 null로 초기화 (항상)
        ...
        // ✅ appBeforeLock은 유지 (재진입 감지용)
        Log.d("StateLog", "🔄 상태 초기화 완료: currentActiveApp=null (was: $previousApp), appBeforeLock 유지=$appBeforeLock")
...
//            }
    ...
    private fun showLockScreen() {
        ...
        if (!checkAndUpdateLockTime(this)) {
